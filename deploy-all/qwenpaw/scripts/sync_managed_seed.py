    ...
    try:
        for source_path in sorted(seed.rglob("*")):
            ...
            if source_path.is_symlink():
                raise ManagedSyncError(
                    f"Refusing symbolic link seed path: {source_path}",
                )
...
def _assert_no_symlinks(root: Path) -> None:
    if root.is_symlink():
        raise ManagedSyncError(f"Refusing symbolic link target: {root}")
    if not root.is_dir():
        return
...
def _managed_file_entries(
    ...
    for raw_relative, entry in manifest["managed_files"].items():
        ...
        if kind == "skill_file" and _is_protected_skill_path(relative):
            raise ManagedSyncError(
                f"Protected path declared managed: {raw_relative}",
            )
...
def _plan(
    ...
    for relative, entry in _managed_file_entries(manifest):
        ...
        if entry["kind"] == "skill_file" and skill_root is not None:
            grouped_skill_files.setdefault(skill_root, []).append(
                (relative, entry),
            )
            continue
    ...
    for skill_root, entries in sorted(grouped_skill_files.items()):
        ...
        source_hash = manifest["managed_skills"][str(skill_root)][
            "tree_sha256"
        ]
    ...
    actions.extend(_retired_skill_actions(manifest, target))
    return actions


def _retired_skill_actions(
    manifest: dict[str, Any],
    target: Path,
) -> list[SyncAction]:
    previous = _read_json(_state_path(target), {}).get("managed_skills", {})
    if not isinstance(previous, dict):
        return []
    retired = set(manifest.get("retired_skills", []))
    current = set(manifest["managed_skills"])
    actions: list[SyncAction] = []
    for raw_root, metadata in previous.items():
        root = _safe_relative(raw_root)
        if root in current or root.name not in retired:
            continue
        if not isinstance(metadata, dict) or not isinstance(
            ...
            continue
        entries = [
            (root / relative, {"sha256": file_hash})
            for relative, file_hash in metadata["files"].items()
            if isinstance(relative, str) and isinstance(file_hash, str)
        ]
        destination = _safe_child(target, str(root))
        if destination.exists() and _tree_hash(
            destination,
            entries,
            ...
            actions.append(
                SyncAction(
                    ...
                    kind="skill_tree",
                    path=str(root),
                    reason="retired_managed_skill",
                    target_hash=metadata.get("tree_sha256"),
                ),
            )
    return actions
...
def _tree_hash(
    ...
    for relative, _entry in sorted(entries):
        source_relative = relative.relative_to(
            _skill_root_for(relative) or relative,
        )
...
def _replace_skill_tree(
    ...
    stage = destination.with_name(
        f".{destination.name}.managed-stage-{uuid4().hex}",
    )
    old = destination.with_name(
        f".{destination.name}.managed-old-{uuid4().hex}",
    )
    try:
        ...
        for relative, _entry in entries:
            ...
            if not source_file.is_file() or source_file.is_symlink():
                raise ManagedSyncError(
                    f"Invalid seed skill file: {source_file}",
                )
...
def _retire_skill_tree(
    ...
    for relative, entry in entries:
        ...
        if _target_hash(path) != entry["sha256"]:
            continue
        _backup_file(path, target, backup_root)
        touched.append(path)
        path.unlink()
...
def _merge_agent_config(seed_file: Path, target_file: Path) -> dict[str, Any]:
    ...
    if not target_file.exists():
        return source
    merged = dict(target)
    for field_name in MANAGED_AGENT_FIELDS:
...
def _merge_skill_manifest(
    ...
    retired_skill_names: set[str] | None = None,
) -> dict[str, Any]:
    ...
    if not isinstance(source_skills, dict) or not isinstance(
        target_skills,
        dict,
    ):
    ...
    for name in managed_skill_names:
        ...
        if isinstance(existing, dict):
            for field_name in PRESERVED_SKILL_ENTRY_FIELDS:
                if field_name in existing:
                    next_entry[field_name] = existing[field_name]
        target_skills[name] = next_entry
    for name in retired_skill_names or set():
        target_skills.pop(name, None)
...
def _report_path(target: Path, seed_id: str) -> Path:
    ...
    return (
        target / SYNC_DIR_NAME / "reports" / f"{timestamp}-{seed_id[:12]}.json"
    )
...
def sync_managed_seed(
    ...
    *,
    ...
    previous_state = _read_json(_state_path(target), {})
    previous_skills = previous_state.get("managed_skills", {})
    retired_actions = [
        action for action in actions if action.action == "retire"
    ]
    retired_entries: dict[Path, list[tuple[Path, dict[str, str]]]] = {}
    retired_names: dict[str, set[str]] = {}
    if isinstance(previous_skills, dict):
        for action in retired_actions:
            metadata = previous_skills.get(action.path)
            if not isinstance(metadata, dict) or not isinstance(
                metadata.get("files"),
                dict,
            ):
            ...
            root = _safe_relative(action.path)
            retired_entries[root] = entries
            workspace = _workspace_for(root)
            if workspace is not None:
                retired_names.setdefault(workspace, set()).add(root.name)
    ...
    changed_skill_manifests = {
        _safe_relative(action.path)
        for action in actions
        if action.kind == "skill_manifest"
        and action.action in {"add", "update"}
    }
    ...
    for relative, entry in _managed_file_entries(manifest):
        ...
        if entry["kind"] == "skill_file" and skill_root is not None:
            ...
            if workspace is not None:
                managed_skill_names.setdefault(workspace, set()).add(
                    skill_root.name,
                )
    ...
    with _file_write_lock(target / ".qwenpaw_restore.lock"):
        ...
        try:
            ...
            for skill_root, entries in retired_entries.items():
                destination = _safe_child(target, str(skill_root))
                _retire_skill_tree(
                    destination,
                    target,
            ...
            for workspace in set(managed_skill_names) | set(retired_names):
                names = managed_skill_names.get(workspace, set())
                manifest_relative = (
                    Path("workspaces") / workspace / "skill.json"
                )
                ...
                _write_skill_manifest(
                    destination,
                    _merge_skill_manifest(
                        source,
                        destination,
                        names,
                        retired_names.get(workspace),
                    ),
                )

            result.backup_path = (
                str(backup_root) if backup_root.exists() else None
            )
            result.report_path = str(selected_report)
            report = result.as_dict()
            _write_json_atomic(selected_report, report)
...
def main() -> int:
    ...
    try:
        ...
        print(f"managed seed sync error: {exc}", file=sys.stderr)
        return 2
