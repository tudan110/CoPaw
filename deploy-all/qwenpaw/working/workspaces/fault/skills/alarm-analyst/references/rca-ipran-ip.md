...
- 🚑 **紧急预案（止血/降损）**：先区分真故障与管理面不可达；真宕机且有主备 / 双归 → 切至备用设备 / 路径恢复业务；主控故障 → 触发主备倒换。
- 🔧 **根因处置（修复）**：整机掉电 → 恢复供电 / 换电源；软件崩溃 → 重启 / 升级；主板或主控故障 → 换板；管理面不可达 → 修管理网络。
- 🧭 **影响范围决策**：核心设备且有冗余 → 先切备恢复业务，再换件 / 抢修；无冗余核心 → 边应急边最快恢复；边缘设备可直接检修。
...
**⚑ 处置建议（先止血 · 后修复，按影响范围决策）**
- 🚑 **紧急预案（止血/降损）**：质差引发丢包 / 误码 → 先将业务切至正常中继 / 成员链路，隔离质差链路。
- 🔧 **根因处置（修复）**：更换劣化光模块、清洁或重熔光纤降衰减、更换 PHY 异常端口、扩容拥塞带宽。
- 🧭 **影响范围决策**：有 LAG / 等价链路 → 先隔离质差成员切换，再换件；无备则边降级承载边抢修。
...
**⚑ 处置建议（先止血 · 后修复，按影响范围决策）**
- 🚑 **紧急预案（止血/降损）**：板卡故障且有冗余板 / 主备 → 切换到正常板卡；温度过高 → 先降温（清散热 / 修风扇）避免连锁。
- 🔧 **根因处置（修复）**：更换故障板卡、修复散热 / 风扇、升级板卡软件、为过载板卡分流扩容。
- 🧭 **影响范围决策**：核心业务板且有冗余 → 先切换再热插拔换板；无冗余 → 安排窗口换板并做业务迁移。
...
**⚑ 处置建议（先止血 · 后修复，按影响范围决策）**
- 🚑 **紧急预案（止血/降损）**：风扇故障致温升 → 加强机房 / 机柜散热、临时降载，防止高温触发保护宕机。
- 🔧 **根因处置（修复）**：更换故障风扇、清理积尘卡阻、修复供电 / 控制板。
- 🧭 **影响范围决策**：温度逼近阈值 → 先物理散热 + 降载止血，再更换风扇（多可热插拔）；温度可控则直接换。
...
**⚑ 处置建议（先止血 · 后修复，按影响范围决策）**
- 🚑 **紧急预案（止血/降损）**：单电源故障但双电源冗余 → 由冗余电源承载，尽快补件恢复冗余；外部供电异常 → 切备路 / UPS。
- 🔧 **根因处置（修复）**：更换损坏电源模块、修复 PDU / 市电、为功耗过载扩容 / 分载、修电源风扇。
- 🧭 **影响范围决策**：冗余电源在位 → 业务不中断，尽快换件恢复冗余（消除单点）；无冗余核心 → 窗口内换件并做迁移。
...
**⚑ 处置建议（先止血 · 后修复，按影响范围决策）**
- 🚑 **紧急预案（止血/降损）**：有备份链路 → 将受影响用户 / 业务迁移到正常链路先恢复；单端硬件 / 光模块故障且核心 → 先切换再换件。
- 🔧 **根因处置（修复）**：光模块故障 / 不匹配 → 更换匹配光模块；光纤中断 → 抢修；对端 Down → 协同对端处置；端口硬件 → 换板。
- 🧭 **影响范围决策**：核心且有冗余 → 先切换迁移用户，再停业务换光模块；无冗余核心 → 边应急承载边最快换件；边缘可直接换件。
...
**⚑ 处置建议（先止血 · 后修复，按影响范围决策）**
- 🚑 **紧急预案（止血/降损）**：端口翻转比断链更隐蔽伤害更大 → 先 shutdown 该抖动端口并切至备份链路，止住反复震荡对业务的冲击。
- 🔧 **根因处置（修复）**：重插 / 更换接触不良或劣化光模块、处理临界光功率（清洁 / 换纤）、更换间歇故障端口、协同对端。
- 🧭 **影响范围决策**：抖动端口承载核心 → 先关抖端口切备止血，再换件；否则可择机处理。
