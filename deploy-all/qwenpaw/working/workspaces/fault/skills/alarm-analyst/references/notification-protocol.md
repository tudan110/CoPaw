...
工单创建成功后，必须执行 webhook 通知推送。通知由 `scripts/create_manual_workorder.py` 内部自动完成。
...
- 根因方向
- 处置建议
- 工单接口返回的 `procInsId` / `taskId`

通知文案里必须明确这是 **AI 创建** 的工单。

## 推送结果展示规则

- 成功：`通知状态：✅ 已成功推送`（不需要逐渠道列出）
- 未配置：明确写出"通知未配置"
- 部分失败：明确写出"工单已创建，但部分通知发送失败"，列出失败渠道和原因
