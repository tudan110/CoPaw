...
| 告警级别 | 建议策略 | 最多条数 | 示例 |
|---------|---------|---------|------|
| **1（严重）** | **只给紧急止损建议**，告诉用户立刻做什么。不要输出短期优化、中期加固等非紧急内容 | 2-3 条 | 检查 InnoDB 锁等待事务并 KILL 阻塞源；检查磁盘 I/O 是否瓶颈 |
| **2（重要）** | 给出紧急处置 + 必要的短期优化建议 | 3-4 条 | 紧急处置 + 分析慢查询日志 |
| **3（一般）** | 给出短期优化建议即可 | 2-3 条 | 优化慢 SQL、调整参数 |
| **4（提示）** | 给出关注建议或预防性措施 | 1-2 条 | 关注趋势变化、定期巡检 |

**禁止事项**：
- 严重告警中不要出现"中期加固""长期规划"类建议
