...
- **止血优先（紧急预案）**：故障影响**核心业务 / 大面积用户 / 无冗余可用**时，处置建议第一条必须是紧急预案——链路或节点切换、流量迁移、主备倒换、熔断降级、限流隔离、配置回滚等，用最快、最稳的动作先把业务恢复到可用，再从容处置根因。
...
8. 影响范围分析（业务等级/用户范围/冗余备份状态——决定处置建议走紧急预案还是直接根治）
9. 推送分析报告 + 通知推送
10. 恢复验证 → 清除告警 → 更新状态
```
...
> **处置建议一律用 `--suggestions-json`（带 `stage` 字段），不要再用 `--suggestion` + 单独的 `--emergency-plan`**：报告正文按「处置建议规则」分了 `### 🚑 紧急预案` / `### 🔧 根因处置` 两节时，就把对应条目分别标 `"stage":"emergency"` / `"stage":"repair"` 传进同一个 `--suggestions-json` 数组——脚本会自动把 `stage=emergency` 的条目汇总成紧急预案、其余条目汇总成处置建议，卡片和飞书推送用的是同一份数据，不会再出现"卡片有、推送没有"。只给根因处置（不启用双轨）时，`stage` 全部写 `"repair"` 或干脆不传 `stage`。`--emergency-plan` 参数仍然存在，仅作手工兜底，正常流程不要用。
...
8. 处置建议（影响范围大时分 `### 🚑 紧急预案` / `### 🔧 根因处置` 两个子节，见 portal-card-protocol.md）
9. 通知推送结果
10. 恢复验证与状态回写
...
### 处置建议规则（先止血 · 后修复，按影响范围决策）

处置建议按「处置总原则」分双轨输出，由第 8 步影响范围分析的结论决定给哪一轨：

**影响范围大（核心业务 / 大面积用户 / 无冗余可用）→ 双轨输出，紧急预案在前**：
- 🚑 **紧急预案（止血/降损）**：最快恢复业务的动作放第一条——切换到备份链路/节点、把受影响用户/业务迁移到正常路径、主备倒换、熔断降级、限流隔离、回滚引发故障的变更、启用备用电源/制冷等。动作要具体到"切到哪、迁到哪、回滚哪个变更"。
- 🔧 **根因处置（修复）**：止血之后的根治动作（更换光模块/板卡、修复线路、扩容、升级版本等），注明可择机安排、避开业务高峰。

**影响范围可控（有有效冗余 / 边缘单点 / 单用户，或紧急预案代价高于收益）→ 只给 🔧 根因处置**，不强行加止血步骤，避免信息过载。

每个场景的具体止血与修复动作，参考对应 `references/rca-*.md` 中该场景末尾的「⚑ 处置建议」块（含 🚑 紧急预案 / 🔧 根因处置 / 🧭 影响范围决策）。
...
- 任何级别的告警中都不要出现"中期加固""长期规划"类建议；除 🚑/🔧 双轨外不要再加"短期优化"等更多层次
