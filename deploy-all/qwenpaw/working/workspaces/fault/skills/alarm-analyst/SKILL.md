...
description: 面向单条活动告警或单个应用故障现象驱动的故障分析与闭环处置技能。适用于 Portal 中用户点击右上角铃铛告警后，或直接描述“某应用新增数据失败 / CMDB 插入数据失败”等现象后，由故障分析员数字员工接管，并向用户展示从智观活动告警、CMDB 资源确认、应用拓扑拆解、指标分析、影响范围判断、处置建议、恢复验证到清除告警/更新工单状态的完整过程。若当前工作区已具备 shell、跨智能体和指标接口能力，应优先执行真实查询，不要只复述流程模板。
---
...
如果最终回复要用于 Portal 的 `alarm-analyst` 主卡展示，必须遵守以下约定：

1. 在完整告警分析报告最前面输出标志行：`# PORTAL ALARM ANALYST CARD MODE`
2. 标志行后紧跟一个单独分隔段：`---`
...
6. `## 📊 总结` 必须保留固定标签：
   ...
   - `关联资源告警查询状态`
   - `通知状态`
7. 只有最终完整报告允许使用这个 marker；单步过程、通知结果、权限失败、脚本回显都不允许套用
...
如果当前这条回复要作为 Portal 主卡的最终完整报告，还必须改成下面这套固定结构：
...
- 工单创建成功后，必须明确体现“已完成通知推送”或“通知失败原因”
- 如果总结里要写推送结果，统一使用“通知状态”，不要写“飞书通知状态”“钉钉通知状态”这类单一渠道措辞
- 推送成功时，优先写 `通知状态：✅ 已成功推送`，不要再展开具体渠道名
