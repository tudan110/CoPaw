...
description: 面向 inspection 智能体的资源巡检技能。优先使用 inspection 工作区本地的 zgops-cmdb 明确巡检对象的拓扑、resId/CI ID 与 ciType；若本地 skill 不可用，再协作 query 智能体。随后查询该资源类型的全部指标定义与指标值，最后向用户展示包含 CMDB 拓扑关系的巡检结果。
---
...
1. 先识别用户要巡检的对象
2. 优先使用 inspection 本地的 `zgops-cmdb` 确认拓扑、资源 ID（CI ID / resId）与资源类型（`ciType` / `metricType`）；本地 skill 不可用时再协作 query
...
1. **优先使用 inspection 智能体当前工作区下已有的本地 skill**
   - `skills/zgops-cmdb`
...
- 查 CMDB / 应用拓扑时，默认先使用 **inspection 本地的 `zgops-cmdb`**
- 只有本地 skill 不可用时，才回退去协作 `query`
...
1. 默认优先使用 inspection 工作区下已有的本地 `zgops-cmdb`
2. 只有本地 `zgops-cmdb` 不可用时，才使用内置工具 `chat_with_agent` 协作 `query` 智能体
3. 让本地 `zgops-cmdb`（或回退协作的 query）明确：
   - 根资源名称
   - `resId / CI ID`
   - `ciType`
   - 基本拓扑关系
...
如果本地 `zgops-cmdb` 或回退协作的 query 返回多个候选资源，不能默认任选一个继续巡检。
...
涉及 CMDB / 拓扑确认时，必须优先使用 inspection 本地的 `zgops-cmdb`，不要只凭用户一句“数据库”就直接假定资源。

只有以下情况才回退到 `query`：

1. inspection 工作区下没有 `zgops-cmdb`
2. 本地 `zgops-cmdb` 缺少配置、接口未接通或执行失败
3. 用户明确要求“让 query 来查”

如果回退到 `query`，优先使用 `chat_with_agent`，并在过程说明中明确写出回退原因。

推荐本地优先提示：

```text
请优先使用 inspection 工作区下的 zgops-cmdb，确认巡检对象“<用户巡检对象>”在 CMDB 中对应的资源信息，返回：
1. 最匹配的根资源名称
2. resId / CI ID
3. ciType
4. 简要拓扑摘要
