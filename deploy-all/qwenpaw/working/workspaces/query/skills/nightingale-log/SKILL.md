...
category: log
tags: [log, observability, elasticsearch, lucene, syslog]
triggers: [日志查询, 日志检索, 查日志, 看下日志, 看一下日志, 看看日志, 最近的日志, 最近日志, 最近N分钟日志, 最近N小时日志, 错误日志, 异常日志, 报错日志, ERROR日志, WARN日志, 日志列表, 日志统计, 日志分布, 日志趋势, 日志变化, 每分钟日志数, 智观日志, 日志服, 日志服务, ES日志, syslog, 业务日志, 应用日志, 系统日志, 服务日志, 日志Top, 关键字日志, 包含xxx的日志, 某主机日志, 某服务日志]
description: 智观日志服务即时查询，对接 ES / casaos-syslog 等业务日志数据源。支持 KQL / Lucene 语法的日志检索、级别分布、主机/服务分布、时间序列直方图与关键字搜索。当用户提到“查日志、看下日志、最近 N 分钟/小时的日志、错误日志、异常日志、报错日志、日志统计、日志分布、日志趋势、按服务/主机统计日志、包含 xxx 的日志、某主机/服务的日志、智观日志/日志服/ES/syslog 检索”等任意日志查询诉求时**必须使用本技能**。本技能查询的是平台接入的**业务/应用/系统日志**（生产线上日志），不是 QwenPaw 智能体自身的运行日志、控制台输出或会话历史。本技能只用于日志数据查询与统计可视化，不用于告警查询（real-alarm）、CMDB 查询（zgops-cmdb）或一般技术问答。
---

# 智观日志查询

为智观日志服务后端的 ElasticSearch 日志查询提供最短执行路径。覆盖：日志列表检索、关键字搜索、按级别 / 主机 / 服务的分布统计、时间直方图、原始命中导出。
...
# 默认日志数据源 ID（在日志服务 “数据查询 -> 日志” 页面看到的 ES 数据源 ID）
...
当用户提到以下诉求时，**必须**使用本技能（不要用其他工具自己读取本机日志或回看会话历史代替）：

- 查日志 / 看下日志 / 看一下日志 / 看看日志 / 显示日志
- 日志列表 / 最近日志 / 最近 N 分钟的日志 / 最近 N 小时的日志 / 今天的日志
- 错误日志 / 异常日志 / 报错日志 / 严重日志 / WARN / ERROR
- 关键字搜索日志（包含 xxx、含 xxx 的日志）
- 日志按级别 / 主机 / 服务 / 应用 / 容器 分布或排行
- 日志趋势 / 日志时间分布 / 每分钟日志数 / 日志变化
- 某主机 / IP / 服务 / 容器 / pod 的日志

**关键澄清**：用户说“看下日志”“最近的日志”默认指**智观日志服务接入的业务/应用日志（ES 数据源，索引如 `casaos-syslog-*`）**，不是：

- ❌ QwenPaw 智能体自己的运行日志
- ❌ 控制台 stdout / 启动日志 / agent reload 事件
- ❌ 当前会话的对话历史 / 工具调用记录
- ❌ 本机 `/var/log` 系统文件

只有用户**明确**说“QwenPaw 的日志”、“智能体的运行日志”、“控制台日志”、“你刚才做了什么”时，才不走本技能。
