...
export const PORTAL_ADVANCED_PANEL_OPTIONS = ["model-config", "token-usage", "ops-expert", "mcp", "skill-pool", "fde-workbench", "knowledge-base", "inspiration", "resource-import", "nl-customization", "ai-big-screen", "app-market", "app-artifacts", "app-workbench", "dashboard-assembly", "proxy-datasources", "cli", "traces", "self-monitor", "settings", "alarm-registry", "channels", "inbox"] as const;
...
function sanitizeRemoteUserText(content: string) {
  ...
  if (
    normalized.includes(PORTAL_ALARM_ANALYST_CARD_MARKER) &&
    normalized.includes("\n---\n")
  ) {
    const segments = normalized.split(/\n---\n/);
    const candidate = segments[segments.length - 1]?.trim();
...
export function unwrapPortalAlarmAnalystCardContent(content: string) {
  const normalized = String(content || "");
  if (
    normalized.includes(PORTAL_ALARM_ANALYST_CARD_MARKER)
    && normalized.includes("\n---\n")
