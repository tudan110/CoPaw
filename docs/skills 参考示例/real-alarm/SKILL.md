...
如果暂时没有接口权限，可以设置 `USE_MOCK_DATA=true`，脚本会改用技能目录下的
`mock_data.json` 返回演示数据，用于先验证参数解析、过滤、渲染逻辑。

未指定 `--begin_time`/`--end_time` 时，脚本按 `REAL_ALARM_QUERY_WINDOW_HOURS`
（默认 24 小时，可在 `.env` 中调整）自动回溯生成查询时间窗——这是接口的强制要求。
...
当用户提到数据库、网络设备、中间件、操作系统、服务器等资源分类时，必须把分类传给 `--ne_alias`（脚本内部会映射为接口字段 `alarmClassType`），不要先查全量再本地过滤。

如果用户没有提到资源分类，不要传 `--ne_alias`，保持全量查询。

| 用户说法 | 推荐参数 | 接口 alarmClassType |
|---------|----------|--------------|
...
**基础查询类**：
- "有多少条告警" → 先查询总数，直接返回 `total`
- "列出告警" → 查询第一页或用户指定数量，表格展示

**筛选查询类**：
- "有哪些严重告警" → `analyze_alarms.py --mode search --severity 1 --include-alarms`
- "活跃告警有哪些" → `analyze_alarms.py --mode search --alarm_status 1 --include-alarms`
- "查询数据库当前告警" → `analyze_alarms.py --mode search --ne_alias 数据库 --alarm_status 1 --include-alarms`
- "某个设备的告警" → `analyze_alarms.py --mode search --device_name <name> --include-alarms`
- "某个 CI ID 的告警" → `analyze_alarms.py --mode search --ci_id <id> --include-alarms`

**统计分析类**：
- "统计告警级别" → `analyze_alarms.py --mode severity`
- "按设备统计告警" → `analyze_alarms.py --mode device`
- "按专业统计告警" → `analyze_alarms.py --mode speciality`
- "告警类型统计" → `analyze_alarms.py --mode title`

**综合分析类**：
- "帮我分析一下告警情况" → `analyze_alarms.py --mode summary`

**时间范围查询**：
- "昨天的告警" / "最近24小时告警" → 使用 `--begin_time` 和 `--end_time` 参数
