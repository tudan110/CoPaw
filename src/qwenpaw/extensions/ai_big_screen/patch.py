...
def _build_patch_messages(
    *,
    ...
    system_prompt = (
        ...
        "scroll auto|off|on(表格/列表的自动轮播滚动)}；"
        ...
        "【创作通道】当用户要新增的是让你【创建/编排/填充】的表格、"
        "清单或文本，而不是接入真实系统的实时数据(不仅乘法表、对照表、"
        "口诀、公式、概念说明，也包括排班表、值班表、组织架构、计划/清单、"
        "示例/样例数据、评分表、日程、路线图、模板等任何你能直接生成的"
        "成品)，addComponent 用 capabilityId=ai-authored-content，"
        "并把你生成的完整内容内联到 queryParams.content："
        "{columns:[{key,label}...], rows:[{...}...]}"
        "(表格类)或{text:说明文字}(文本类)或{metrics:{名:值}}(数值类)——"
        "内容必须完整可用，后端不会访问任何外部源；行数多(超过50行)时"
        "只保留最关键的3-4列但行数必须完整，绝不截断或用省略号。"
        "不要因为'没有现成数据源'就把用户让你创作的内容标成待接入。"
        "ai-authored-content 绝不可用于告警/工单/CMDB/资源/日志/监控等"
        "真实运维遥测(这类必须绑真实能力，没有对应能力才诚实缺口，严禁编造)。"
