...
def _build_intent_messages(prompt: str, title: str) -> list[dict[str, str]]:
    ...
    system_prompt = (
        ...
        "当用户是让你【创建/编排/填充】一张表格、清单或一段文本，"
        "而不是接入某个真实系统的实时数据时(不仅包括乘法表、对照表、"
        "口诀、公式、概念说明这类知识，也包括排班表、值班表、组织架构、"
        "计划/清单、示例/样例数据、评分表、日程、路线图、模板等任何"
        "你能直接把内容生成出来的成品)，一律使用 "
        ...
        "关键：不要因为'没有现成数据源'就把用户明明让你创作的内容"
        "(如上面这些)标成 capability-gap 待接入——你自己就能生成，"
        "capability-gap 只留给'用户要真实的内部运维/业务系统数据"
        "但确实没有对应能力'的情况。"
